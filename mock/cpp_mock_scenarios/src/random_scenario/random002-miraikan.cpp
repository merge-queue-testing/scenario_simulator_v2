...
public:
  ...
  : cpp_mock_scenarios::CppScenarioNode(
      "random_miraikan", std::string(getenv("HOME")) + "/workspace/large_size_odaiba_beta",
...
private:
  ...
  void onUpdate() override
  {
    ...
    // parked vehicle (teleport station)
    spawnRoadParkingVehicles(300087, randomInt(1, 2), DIRECTION::LEFT);
    ...
    //   spawnAndChangeLane(
    //     "lane_following_0", onstructLaneletPose(34513, 0.0), 34684, Direction::RIGHT);
    // }
  ...
  void onInitialize() override
  {
    ...
    std::vector<lanelet::Id> route1 = {spawn_goal_lane_id_};
    route_.emplace(spawn_start_lane_id_, true, route1, true);

    std::vector<lanelet::Id> route2 = {83};
    route_.emplace(std::nullopt, true, route2, true);

    std::vector<lanelet::Id> route3 = {300087};
    route_.emplace(175965, true, route3, true);
