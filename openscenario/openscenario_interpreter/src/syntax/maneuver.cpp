...
auto operator<<(rabbit::object & json, const Maneuver & maneuver) -> rabbit::object &
{
  json["name"] = std::move(maneuver.name);

  json["currentState"].set(boost::lexical_cast<std::string>(maneuver.state()));

  json.insert("Event", rabbit::array());

  for (const auto & event : maneuver.elements) {
    rabbit::object json_event;
    json_event << event.as<Event>();
    json["Event"].push_back(std::move(json_event));
  }
